...
PM_patientRecord::PM_patientRecord(QObject *parent): QObject (parent)
{

}

PM_patientRecord::PM_patientRecord(uint patient_UID, QObject *parent): QObject (parent)
{
...
bool PM_patientRecord::initWithUID(uint patient_UID)
{

}

PM_patientRecord::PatientCard PM_patientRecord::readPatientCard(const QJsonObject &json)
{
    PatientCard card;
    card.ID = json["ID"].toString().toUInt();
    card.UID = json["UID"].toString().toUInt();
    card.fullName = json["fullName"].toString();
    return card;
}

void PM_patientRecord::writePatientCard(QJsonObject &json, const PM_patientRecord::PatientCard card)
{
    json["ID"] = QString().number(card.ID);
    json["UID"] = QString().number(card.UID);
    json["fullName"] = card.fullName;
}
